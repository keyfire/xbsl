...
| `xbsl.linter.enable` | – | Включить правила, выключенные по умолчанию (идентификаторы, группы или буквы тиров `A`–`D`). В отличие от `select` стандартный набор остаётся – эти правила добавляются к нему. Другого способа включить такое правило нет: `xbsl.rules` – оверлей поверх находок, которые сервер уже прислал, а невыполнявшееся правило находок не даёт. |
...
| `xbsl.deploy.envFile` | – | `.env` с подключением и целью, передаётся как `--env-file` (относительно папки воркспейса или абсолютный); удобно в git-worktree, где `.env` лежит в основном каталоге. |
