...
возможностей это безразлично; [словарю перевода](#словарь-перевода) нужен **xbsl 0.71.0 или
новее** – команды `translate --entries / --gaps / --set`, на которых он построен, появились в
0.70.0, а третий план словаря, строковые **литералы**, – в 0.71.0. Движок 0.70.0 нарисовал бы
имена и строки комментариев и промолчал о литералах: словарь выглядел бы полным над исходниками,
в которых остались кириллические сообщения. С движком постарше панель не открывается и сообщает,
какая версия установлена (`pip install -U xbsl`).
...
(`xbsl.translate.dictionary`) открывает его таблицей – вид (имя, строка комментария или литерал),
ключ, перевод, сколько раз он встречается в исходниках, первое место и файл словаря, где лежит
запись.

- **Ячейка перевода редактируется.** Введённое пишет движок (`xbsl translate --set`) – в нужный
  файл и с нужной областью; очищенная ячейка удаляет запись. Отказ показывает сообщение движка и
...
- **Литерал пишется так, как он стоит в исходнике** – текст между кавычками, кавычка внутри как
  ...
  времени; отказ показывается вместе с причиной, а ячейка остаётся с прежним значением.
- **Два отбора**: строка поиска по ключам и переводам и *только незаполненные* – то, что словарь
  ...
  литералов.
- **Платформенное написание** стоит серым в пустой ячейке, если движок его предлагает; по щелчку
  оно подставляется и записывается. Литералу такого написания не бывает: таблицы платформы
  называют имена, а между кавычками чаще стоит целое сообщение.
- **Место – ссылка**: открывает файл исходника на этой строке; файл словаря открывается на самой
  записи.
- Строка над таблицей считает строки, незаполненные среди них и покрытие проекта – то самое число,
  которое отдаёт `xbsl translate` и по которому гейтит CI. Литералы посчитаны рядом, как их считает
