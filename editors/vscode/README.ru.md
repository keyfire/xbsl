...
![XBSL в работе: слева дерево метаданных и палитра компонентов, в центре конструктор формы – структура, данные и каркас, справа панели свойств и документации, снизу исходник yaml](https://raw.githubusercontent.com/keyfire/xbsl/main/editors/vscode/images/overview.ru.png)
...
| `xbsl.linter.select` | – | Только эти правила (идентификаторы, группы или буквы тиров `A`–`D`). **Устарело** – то же самое задаётся таблицей `xbsl.rules`; перенести – команда *XBSL: перенести настройки правил в одну таблицу*. |
| `xbsl.linter.ignore` | – | Исключить эти правила. **Устарело** – то же самое задаётся таблицей `xbsl.rules`; перенести – команда *XBSL: перенести настройки правил в одну таблицу*. |
| `xbsl.linter.enable` | – | Включить правила, выключенные по умолчанию (идентификаторы, группы или буквы тиров `A`–`D`). В отличие от `select` стандартный набор остаётся – эти правила добавляются к нему. Другого способа включить такое правило нет: `xbsl.rules` – оверлей поверх находок, которые сервер уже прислал, а невыполнявшееся правило находок не даёт. **Устарело** – то же самое задаётся таблицей `xbsl.rules`; перенести – команда *XBSL: перенести настройки правил в одну таблицу*. |
| `xbsl.rules` | `{}` | **Единая таблица правил.** Ключ – правило (`code/brackets`), группа (`style`), буква тира (`A`) или `*`; значение – `off` или уровень. Приоритет: правило → группа → тир → `*`. Ключ с уровнем ВКЛЮЧАЕТ правило, выключенное по умолчанию; `{"*": "off"}` – "только перечисленные". См. [Правила](#правила-уровни-и-отключение). |
...
| `xbsl.groups.*` | `default` | Выпадающий список на каждую группу правил (code, yaml, project, naming, style, typography, whitespace, encoding, structure, form, query, security, conventions): собственные уровни правил, единый уровень на всю группу или `off`. Группа **naming** – имена элементов проекта по стандарту платформы (нужен `xbsl` >= 0.11.0). См. [Правила](#правила-уровни-и-отключение). **Устарело** – то же самое задаётся таблицей `xbsl.rules`; перенести – команда *XBSL: перенести настройки правил в одну таблицу*. |
...
| --- | --- | --- |
| XBSL: проверить весь проект | `xbsl.lintProject` | палитра команд |
| XBSL: новый проект 1С:Элемент | `xbsl.project.new` | палитра команд |
| XBSL: структурный поиск по формам | `xbsl.forms.search` | палитра команд |
| XBSL: перезапустить линтер | `xbsl.restartLinter` | палитра команд |
| XBSL: палитра кода | `xbsl.choosePalette` | палитра команд |
| XBSL: деплой на стенд (elemctl) | `xbsl.deploy` | палитра команд |
| Проверить, вышло ли новое расширение | `xbsl.checkForUpdate` | палитра команд |
| XBSL: конструктор формы | `xbsl.previewForm` | палитра команд |
| XBSL: перейти к определению или к его документации | `xbsl.goToDefinition` | палитра команд |

**Шаблоны кода**

| Команда | Идентификатор | Откуда вызывается |
| --- | --- | --- |
| XBSL: шаблоны кода | `xbsl.templates.manage` | палитра команд |
| XBSL: импорт шаблонов кода | `xbsl.templates.import` | палитра команд |
| XBSL: экспорт шаблонов кода | `xbsl.templates.export` | палитра команд |
...
| Команда | Идентификатор | Откуда вызывается |
| --- | --- | --- |
| XBSL: обновить дерево метаданных | `xbsl.metadata.refresh` | палитра команд |
...
| XBSL: группировка дерева (по классам / по подсистемам) | `xbsl.metadata.groupMode` | палитра команд |
...
| Команда | Идентификатор | Откуда вызывается |
| --- | --- | --- |
| XBSL: обновить структуру формы | `xbsl.formStructure.refresh` | палитра команд |
...
| XBSL: обновить палитру компонентов | `xbsl.formPalette.refresh` | палитра команд |
...
| XBSL: обновить панель данных | `xbsl.formData.refresh` | палитра команд |
...
| Команда | Идентификатор | Откуда вызывается |
| --- | --- | --- |
| XBSL: поиск по документации | `xbsl.docs.search` | палитра команд |
| XBSL: документация по символу | `xbsl.docs.showForSymbol` | палитра команд |
| XBSL: обновить дерево документации | `xbsl.docs.refresh` | палитра команд |
| XBSL: открыть страницу документации | `xbsl.docs.open` | панель / контекстное меню |
