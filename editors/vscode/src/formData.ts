...
export class FormDataModel {
  ...
  async load(): Promise<void> {
    ...
    if (!uri) {
      this.records = undefined;
      this.message = vscode.l10n.t("Open a form yaml (InterfaceComponent) – the data panel follows the active editor.");
      this.publish();
      return;
    }
    if (!lspActive()) {
  ...
  async addProperty(): Promise<void> {
    if (!this.records) {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("Open a form yaml (InterfaceComponent) – the data panel follows the active editor.")
      );
      return;
    }
