...
const META_KIND_RE = /^(?:ВидЭлемента|ElementKind)[ \t]*:/m;
...
export function classifyEditor(languageId: string, fileName: string, head: string, full: string): EditorMode {
  ...
  const component = head.includes("КомпонентИнтерфейса") || head.includes("InterfaceComponent");
  if (component && (full.includes("Наследует") || full.includes("Inherits"))) {
    return "component";
  }
...
const META_KIND_VALUE_RE = /^(?:ВидЭлемента|ElementKind)[ \t]*:[ \t]*["']?([^"'\s#]+)/m;
...
// The current value merged into a closed option list: the select must be able to show what
// the file already says, even when it is not a valid candidate.
function withCurrentValue(value: string, candidates: string[] | undefined): string[] | undefined {
  if (!candidates || candidates.length === 0) {
    return undefined;
  }
  return value && !candidates.includes(value) ? [value, ...candidates] : candidates;
}
...
function schemaRowEditor(
  ...
  switch (prop.kind) {
    ...
    case "string":
      ...
      if (prop.type === "AttributeName" && attrOptions) {
        return { control: "enum", options: attrOptions };
      }
      return { control: "text", multiline: false };
    default:
      return { control: "readonly" };
...
function schemaRow(
  ...
  typeCandidates: string[] | undefined,
  attrCandidates: string[] | undefined
): PanelRow {
  ...
  return {
    ...
    editor: readonly
      ? { control: "readonly" }
      : schemaRowEditor(prop, schema, typeCandidates, withCurrentValue(value, attrCandidates)),
  ...
  const toRow = (r: MetaPropRow): PanelRow => {
    ...
    return {
      ...
      editor: prop && !r.readonly && SCALAR_PROP_KINDS.has(prop.kind ?? "")
        ? schemaRowEditor(
            prop, schema as MetaSchema, typeCandidates, withCurrentValue(r.value, attrCandidates),
          )
