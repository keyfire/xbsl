...
class FormPaletteProvider implements vscode.TreeDataProvider<PaletteElement> {
  ...
  async rebuild(): Promise<void> {
    ...
    // The group titles follow the editor language: an English editor gets the English
    // package names the engine ships (see uiterms), a Russian one keeps the schema's own.
    const english = vscode.env.language.toLowerCase().startsWith("en");
    this.sections = buildPalette(catalog, project, this.favorites(), this.usage(), english);
