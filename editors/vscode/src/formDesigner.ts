...
class Designer implements StructureHost, DataHost {
  ...
  async renderFrame(): Promise<void> {
    ...
    const text = doc.getText();
    // The pairs of the structure keys: a form written in English is read by the platform the
  ...
  if (!docUri) {
    ...
    if (!editor || editor.document.languageId !== "yaml") {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("XBSL: open a form yaml (InterfaceComponent) to design it.")
      );
      return;
    }
