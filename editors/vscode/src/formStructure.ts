...
export class FormStructureModel {
  ...
  async load(): Promise<void> {
    ...
    if (!uri) {
      this.index = undefined;
      this.message = vscode.l10n.t("Open a form yaml (InterfaceComponent) – the structure follows the active editor.");
      this.publish();
      return;
    }
    if (!lspActive()) {
    ...
    if (!res || !res.available || !res.root) {
      this.index = undefined;
      this.message = res?.reason || vscode.l10n.t("No form tree here – open a form yaml (InterfaceComponent).");
      this.publish();
      return;
    }
  ...
  async insertPreset(id?: string): Promise<void> {
    if (!this.index || !this.target) {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("XBSL: open a form yaml (InterfaceComponent) – a block preset inserts into the structure selection.")
      );
      return;
    }
  ...
  async pasteFromClipboard(id?: string): Promise<void> {
    if (!this.index || !this.target) {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("XBSL: open a form yaml (InterfaceComponent) – the clipboard yaml is pasted into the structure selection.")
      );
      return;
    }
  ...
  async insertComponentType(type: string): Promise<boolean> {
    if (!this.index || !this.target) {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("XBSL: open a form yaml (InterfaceComponent) – the palette inserts into the structure selection.")
      );
      return false;
    }
  ...
  async insertFragment(fragment: string): Promise<boolean> {
    if (!this.index || !this.target) {
      void vscode.window.showInformationMessage(
        vscode.l10n.t("XBSL: open a form yaml (InterfaceComponent) – the field is inserted into the structure selection.")
      );
      return false;
    }
    const plan = insertPlanForSelection(this.selected()[0], this.index, (t) => cachedContainerTypes()?.has(t) ?? false);
