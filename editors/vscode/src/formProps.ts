...
// A binding sample in the language of the editor: `=Объект.Поле` / `=Object.Field`.
export function bindingSample(): string {
  return vscode.env.language.toLowerCase().startsWith("ru") ? "=Объект.Поле" : "=Object.Field";
}
...
function labels(): Record<string, string> {
  return {
    ...
    // The sample follows the display language: the platform names the object and the field
    // in both languages, and an English hint telling the reader to write `=Объект.Поле` sends
    // them looking for keys their own sources do not use.
...
<script nonce="${nonce}">
  ...
  function bindingCell(row, initial, toLiteral) {
    ...
    input.value = initial;
    input.placeholder = (L && L.bindingSample) || "=Объект.Поле";
    const list = el("div", "combo-list");
    list.style.display = "none";
...
async function refreshMetadata(uri: vscode.Uri, sel: MetaSelector): Promise<void> {
  ...
  // An AttributeName property (the Представление of a catalog/document) offers the object's
