...
| `xbsl.linter.select` | – | Only these rules (ids, groups, or tier letters `A`–`D`). **Deprecated** - the same is said by the `xbsl.rules` table; move it with *XBSL: move the rule settings into one table*. |
| `xbsl.linter.ignore` | – | Exclude these rules. **Deprecated** - the same is said by the `xbsl.rules` table; move it with *XBSL: move the rule settings into one table*. |
| `xbsl.linter.enable` | – | Turn ON rules that are off by default (ids, groups, or tier letters `A`–`D`). Unlike `select` the default set stays in place – these are added to it. This is the only way to enable such a rule: `xbsl.rules` is an overlay over the findings the server already produced, and a rule that never ran produces none. **Deprecated** - the same is said by the `xbsl.rules` table; move it with *XBSL: move the rule settings into one table*. |
