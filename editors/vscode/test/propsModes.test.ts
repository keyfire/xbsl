...
test("buildMetaPanelModel: an AttributeName property becomes a dropdown of string attributes", () => {
  const internals = parseInternals(CATALOG)!;
  const model = buildMetaPanelModel(
    describeMetaSelection(CATALOG, { offset: internals.rootOffset })!,
    undefined,
  ...
  const all = Object.fromEntries(model.sections[1].rows.map((r) => [r.key, r]));
  assert.strictEqual(all["Представление"].editor.control, "enum");
  assert.deepStrictEqual(
    (all["Представление"].editor as { options: string[] }).options,
    ["Описание", "Наименование"]
  );
  // Plain string properties are untouched by the candidates.
  assert.ok(!("Разработчик" in all)); // deprecated - still skipped
});

test("buildMetaPanelModel: the set AttributeName value stays visible even when invalid", () => {
  // The file says Представление: Задача, the object has no such attribute: the select must
  // show the current value (first) so the panel does not lie about the file.
  const doc = CATALOG.replace("Имя: Товары\n", "Имя: Товары\nПредставление: Задача\n");
  const internals = parseInternals(doc)!;
  const model = buildMetaPanelModel(
    describeMetaSelection(doc, { offset: internals.rootOffset })!,
    undefined,
    CATALOG_SCHEMA,
    ["Описание"]
  );
  const row = model.sections
    .flatMap((s) => s.rows)
    .find((r) => r.key === "Представление")!;
  assert.strictEqual(row.editor.control, "enum");
  assert.deepStrictEqual((row.editor as { options: string[] }).options, ["Задача", "Описание"]);
  assert.strictEqual(row.value, "Задача");
});

test("buildMetaPanelModel: no string attributes keep the free text editor", () => {
  const internals = parseInternals(CATALOG)!;
  const model = buildMetaPanelModel(
    describeMetaSelection(CATALOG, { offset: internals.rootOffset })!,
    undefined,
...
test("buildMetaPanelModel: a URL template is editable in the panel", () => {
  ...
  // A handler property is a handler row at every level - here it is the template's AnyMethod.
