...
def test_property_add_creates_section_after_inherit():
    ...
    # the accepted placement: the section opens right after the Наследует block
...
# --- the cardinality of a slot decides its spelling (Массив<...> -> the "-" list form) ---
...
def test_missing_array_slot_is_written_as_a_list():
    # Группа.Содержимое is Массив<Компонент>: a single nested mapping does not compile there
    res = formedits.insert_component(GROUP_ROOT, "Наследует", "Содержимое", type_="Таблица")
    assert "    Содержимое:\n        -\n            Тип: Таблица\n" in res.new_text
    form = parse_form(res.new_text)
    assert form.nodes["Наследует/Содержимое"].list_style is True
    assert res.node_id == "Наследует/Содержимое[0]"


@pytest.mark.needs_data
def test_empty_array_slot_is_filled_with_a_list_item():
    res = formedits.insert_component(
        GROUP_ROOT + "    Содержимое:\n", "Наследует", "Содержимое", type_="Таблица",
    )
    assert "    Содержимое:\n        -\n            Тип: Таблица\n" in res.new_text
    assert parse_form(res.new_text).nodes["Наследует/Содержимое"].list_style is True


@pytest.mark.needs_data
def test_single_typed_slot_keeps_the_mapping_spelling():
    # Форма.Содержимое is one ШаблонФормы - the historical singleton spelling stays
    empty = GROUP_ROOT.replace("Тип: Группа", "Тип: Форма")
    res = formedits.insert_component(
        empty, "Наследует", "Содержимое", type_="ПроизвольныйШаблонФормы",
    )
    assert "    Содержимое:\n        Тип: ПроизвольныйШаблонФормы\n" in res.new_text
    assert parse_form(res.new_text).nodes["Наследует/Содержимое"].list_style is False


@pytest.mark.needs_data
def test_move_into_a_new_array_slot_lands_as_a_list_item():
    # the reported case: a table moved into a fresh Группа became a singleton and broke the build
    source = GROUP_ROOT + (
        "    Содержимое:\n"
        "        -\n"
        "            Тип: Таблица\n"
    ...
    res = formedits.move_node(
        source, "Наследует/Содержимое[0]", "Наследует/Содержимое[1]", "Содержимое",
    )
    form = parse_form(res.new_text)
