...
the server apply with `Неизвестный метод "Массив<Программы.МобПриложения>.Количество"`.
"""
...
_FORM = engine.load_text(
    "ПрограммыФормаОбъекта.yaml",
    "ВидЭлемента: КомпонентИнтерфейса\n"
    "Ид: 019ef4c8-232f-7f33-9da6-c3604720b3ad\n"
    "Имя: ПрограммыФормаОбъекта\n"
    "Наследует:\n"
    "    Тип: ФормаОбъекта<Программы.Объект>\n",
)
...
def test_object_chain_in_form_module_flagged():
    ...
    try:
        d = _lint(_ENTITY, _FORM, _form_module(
            "    если Объект.МобПриложения.Количество() > 4\n        возврат\n    ;\n"))
        assert len(d) == 1
        assert d[0].rule_id == RULE and d[0].severity.value == "error"
        assert "Массив<Программы.МобПриложения>" in d[0].message
...
def test_known_member_silent():
    assert _lint(_ENTITY, _FORM, _form_module(
        "    если Объект.МобПриложения.Размер() > 4\n        возврат\n    ;\n")) == []
...
def test_bare_section_in_entity_module_flagged():
    d = _lint(_ENTITY, engine.load_text(
        ...
        "метод ПередЗаписью()\n"
        "    если МобПриложения.Количество() > 0\n"
        "        знч Всего = этот.МобПриложения.Размер()\n"
        "    ;\n"
        ";\n",
    ))
...
def test_this_chain_flagged():
    d = _lint(_ENTITY, engine.load_text(
        "Программы.Объект.xbsl",
        "метод ПередЗаписью()\n    знч Всего = этот.МобПриложения.Количество()\n;\n",
    ))
    assert len(d) == 1 and d[0].line == 2
...
def test_attribute_is_not_judged():
    ...
    assert _lint(_ENTITY, _FORM, _form_module(
        "    если Объект.Магазины.Количество() > 4\n        возврат\n    ;\n")) == []
...
def test_rebound_name_silences():
    ...
    assert _lint(_ENTITY, engine.load_text(
        "Программы.Объект.xbsl",
        "метод ПередЗаписью(МобПриложения: Массив<Строка>)\n"
        "    если МобПриложения.Количество() > 0\n        возврат\n    ;\n;\n",
    )) == []


def test_module_named_after_section_shadows():
    """Living BizKub code keeps a module called after a section - the bare name is the
    module there, and its methods are no business of the array catalog."""
    d = _lint(_ENTITY,
              engine.load_text("МобПриложения.xbsl",
                               "@ВПроекте\nметод Показать(Ид: Строка): Строка\n"
                               "    возврат Ид\n;\n"),
              engine.load_text("Программы.xbsl",
                               "метод Опубликовать()\n"
                               "    знч Витрина = МобПриложения.Показать(\"м1\")\n;\n"))
    assert d == []
