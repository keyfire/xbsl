...
#: An object that localizes its own caption establishes the project's intent.
...
def test_same_key_on_other_element_kind_silent():
    """Intent is tracked per element kind: namesake properties on different kinds differ.

    In the live case, a component-card description is localized while an event description is
    operator documentation. The latter must not be judged by the former's intent.
    """
    d = _lint(
