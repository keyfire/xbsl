...
поиск по документации платформы, скаффолдинг метаданных, перевод исходников на английские
написания и MCP-сервер для ИИ-агентов – плюс расширение VS Code на том же движке.
...
**Шаг 2 – сгенерировать данные о языке.** Линтер работает по таблицам, извлекаемым из
**вашего** дистрибутива 1С:Элемент (ключевые слова, каталог типов stdlib, метамодель
конфигурации, термины, индекс документации, схема компонентов); в репозиторий они НЕ включены:

```sh
xbsl extract --dist "<каталог дистрибутива 1С:Элемент>"
```

Команда прогоняет все экстракторы разом (из клона то же самое – `python tools/extract.py`,
...
**Правила.** 171 правило базового набора, в четырёх тирах: **A** – структура и схема yaml, **B** – текст и
...
**Базлайн.** Правило можно включить на старом коде, не разгребая его весь сразу: текущие
находки замораживаются один раз, спрос – только с нового кода. Тот же файл хранит точечные
...
метаданных и кнопка деплоя. Работает на `xbsl-lsp` – это Language Server, его поднимет любой
редактор с поддержкой LSP ([подробнее](https://github.com/keyfire/xbsl/blob/main/docs/servers.ru.md#lsp-сервер)).
...
**CI.** Ненулевой код возврата – только при находках уровня error, поэтому `xbsl` встаёт в
конвейер как есть; `--format codeclimate` питает виджет GitLab Code Quality. Готовые джобы
