...
# --- a rule that crashes ------------------------------------------------------------------
#
# A rule is code like any other, and a bug in one of them used to take the WHOLE run down: the
# report showed a traceback instead of the findings, and the tool looked broken rather than
# incomplete (0.36.0: an attribute walk over a parser node compiled by mypyc). A crash is now
# one diagnostic and the rest of the run goes on. The finding carries the id of the failing
# rule, so --ignore over that id silences it like any other finding of that rule.


def _crash_diag(rule_id: str, path: str, exc: Exception) -> Diagnostic:
    """The report of a crashed rule; environment failures go on up instead (see below)."""
...
def _rule_diags(
    info: RuleInfo, path: str, produce: Callable[[], Iterable[Diagnostic]]
) -> list[Diagnostic]:
    """Findings of one rule; its crash becomes a finding instead of the end of the run."""
    try:
        return list(produce())
    except Exception as exc:  # noqa: BLE001 - a rule bug must not take the run down
        return [_crash_diag(info.id, path, exc)]


def _map_facts(
    info: RuleInfo, sources: list[SourceFile], diags: list[Diagnostic]
) -> dict[str, object]:
    """Per-file facts of a map-reduce rule; a crash on one file is reported and that file skipped."""
    facts: dict[str, object] = {}
    for src in sources:
        try:
            fact = info.mapper(src)  # type: ignore[misc] - only called when mapper is not None
        except Exception as exc:  # noqa: BLE001
            diags.append(_crash_diag(info.id, src.rel, exc))
            continue
        if fact is not None:
            facts[src.rel] = fact
    return facts
...
def _worker_lint(payload: tuple) -> tuple[list[Diagnostic], dict[str, dict[str, object]]]:
    ...
    if kind == "file" and mapped_ids:
        ...
        for rule_id in mapped_ids:
            ...
            if info is None:
                continue
            facts[rule_id] = _map_facts(info, sources, diags)
    return diags, facts
...
def run_parallel(
    ...
    *,
    ...
    # and that cost dominates - in measurements 8 groups run twice as SLOW
    ...
    project_path = str(paths[0]) if paths else ""
    for r in mapped_rules:
        diags.extend(_rule_diags(r, project_path, lambda r=r: r.func(merged[r.id])))
    if SEVERITY_OVERRIDES:
        diags = [
            replace(d, severity=SEVERITY_OVERRIDES[d.rule_id])
            if d.rule_id in SEVERITY_OVERRIDES and d.severity != SEVERITY_OVERRIDES[d.rule_id]
