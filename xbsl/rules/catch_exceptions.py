...
A stdlib exception is recognized by the type hierarchy: the catalog stores the whole
ancestor chain of every documented type, so "inherits Исключение" is the platform's own
answer. Both name forms are covered, the root itself included.
"""
...
#: The root of the exception hierarchy, in both name forms.
_EXC_ROOTS = frozenset({"Исключение", "Exception"})
#: Fallback signature of an exception, used only when the catalog carries no hierarchy
#: (data generated by an older extractor): properties every exception page lists.
...
def _stdlib_sets() -> tuple[frozenset[str], frozenset[str]]:
    """Known stdlib type names and those of them that are exceptions.

    An exception is a type that INHERITS Исключение - the catalog carries the whole
    ancestor chain of every page, so this is the platform's own answer rather than a
    guess. The property signature stays as a fallback for datasets without `bases`: it
    happens to select the same 200 types today, but it would also accept any future type
    that merely looks like an exception.
    """
    global _cache
    if _cache is None:
        try:
        ...
        if bases:
            exceptions = frozenset(
                ...
                if name in _EXC_ROOTS or _EXC_ROOTS & set(bases.get(name) or ())
            )
        else:
            exceptions = frozenset(
