...
The yaml/unexpected-type-argument rule. A generic written with an argument is ANOTHER type, and
...
MESSAGES = {
    ...
    "yaml/unexpected-type-argument.title": {
        "ru": "Лишний параметр типа",
        "en": "An unexpected type argument",
    },
    "yaml/unexpected-type-argument.extra": {
        "ru": "Свойство '{prop}' компонента '{component}' объявлено типом '{head}' без параметра "
              "(это '{head}<{default}>'), а здесь записано '{value}' – другой тип, применение "
              "сборки упадёт: 'не может быть присвоено в {head}'. Оставьте '{head}'.",
        "en": "Property '{prop}' of component '{component}' is declared as '{head}' with no "
              "argument (that is '{head}<{default}>'), and this is '{value}' - another type; "
              "applying the build fails with 'не может быть присвоено в {head}'. Write '{head}'.",
    },
    "yaml/no-expression-in-literal.binding": {
        ...
        "en": "Property '{prop}' of a '{type}' node is given as an expression – the platform "
              ...
              "computed expression'). Compute the WHOLE object instead: '{owner}: ={n[Выражение]}'.",
    },
}
...
def _listed(allowed: frozenset[str], value: str) -> frozenset[str]:
    """The allowed values worth showing next to what was written.

    Both spellings are accepted, but listing them all mixes the languages in one line; the hint
    keeps the language of the value the author typed, and falls back to the whole set when that
    leaves nothing (a value that exists in one spelling only).
    """
    latin = value.isascii()
    same = frozenset(v for v in allowed if v.isascii() == latin)
    return same or allowed


def _english_value(value: str) -> str | None:
    """The English spelling of an enumeration value, or None when the platform names none."""
    return terms.english(value, "enums") or terms.common_english(value)
...
def _enumerated_props() -> tuple[dict[str, dict[str, frozenset[str]]], "re.Pattern | None"]:
    ...
    for component, rec in (schema.get("components") or {}).items():
        ...
        for prop, info in (rec.get("props") or {}).items():
            ...
            if allowed is not None:
                # A form may be written in English - the platform reads it either way - so both
                # spellings of the property and of every value it allows are accepted. The pairs
                # are taken FORWARD from the names the schema itself carries: the dictionaries are
                # many-to-one in reverse and would answer with whichever pair came last.
                # The English spellings come from uiterms.json (extracted from the
                # distribution) - the documentation the ui schema is built from is Russian
                # only. The dictionary stays the fallback for what the extractor misses.
                by_enum: dict[str, str] = {}
                for member in info.get("types") or ():
                    by_enum.update(uischema.enum_value_aliases(str(member).strip().rstrip("?")))
                pairs = {value: by_enum.get(value) or _english_value(value) for value in allowed}
                allowed = allowed | {english for english in pairs.values() if english}
                if any(english is None for value, english in pairs.items() if not value.isascii()):
                    # The data does not name every value in English. `DisplayKind: Banner`,
                    # `WidthInColumns: Double`, `Kind: Main` are legal code whose spellings the
                    # dictionary simply does not carry. While the English
                    # half of a set is incomplete, an ASCII value there cannot be judged: it would
                    # be a false error on a legal English source. Russian values stay judged.
                    incomplete.add(prop)
                    english_incomplete = terms.common_english(prop)
                    if english_incomplete:
                        incomplete.add(english_incomplete)
                judged[prop] = allowed
                names.add(prop)
                english_prop = terms.common_english(prop)
                if english_prop:
                    judged[english_prop] = allowed
                    names.add(english_prop)
        if judged:
            table[component] = judged
            english_component = terms.english(component, "types") or terms.common_english(component)
    ...
    _INCOMPLETE_ENGLISH.clear()
    _INCOMPLETE_ENGLISH.update(incomplete)
    return table, keys_re


#: Properties whose value set the data does not spell in English in full - filled by
#: _enumerated_props, read by the rule to stay silent on an ASCII value there.
_INCOMPLETE_ENGLISH: set[str] = set()
...
def unknown_enum_value(source: SourceFile) -> Iterable[Diagnostic]:
    ...
    for mapping in _mapping_nodes(root):
        ...
        for key, (_key_node, value_node) in entries.items():
            ...
            if value.isascii() and key in _INCOMPLETE_ENGLISH:
                continue  # the English half of this value set is unknown (see _enumerated_props)
            yield Diagnostic(
                source.rel,
                ...
                i18n.t(
                    ...
                    allowed=", ".join(sorted(_listed(allowed, value))),
                ),
            )
...
def _default_arguments(record: dict) -> str | None:
    """The default argument list of a generic type, or None when the docs do not state it all.

    `ФрагментКомандногоИнтерфейса` is `ФрагментКомандногоИнтерфейса<Команда>`: the docs give
    ТипКоманды the default Команда. A type whose every parameter has a documented default can
    be compared with what a source writes; one missing default and the answer is unknown, so
    the rule keeps quiet rather than judging by a guess.
    """
    params = record.get("params") or ()
    defaults = record.get("defaults") or {}
    if not params or any(name not in defaults for name in params):
        return None
    return ",".join(defaults[name] for name in params)
...
def _unparametrized_props() -> tuple[dict[str, dict[str, frozenset[str]]], dict[str, str]]:
    ...
    for component, rec in (schema.get("components") or {}).items():
        ...
        for prop, info in (rec.get("props") or {}).items():
            members = [str(t).strip().rstrip("?") for t in (info.get("types") or ())]
            bare = {m for m in members if m in defaults}
            bare -= {m.split("<", 1)[0] for m in members if "<" in m}
            if bare:
...
@rule(
    "yaml/unexpected-type-argument", "yaml/unexpected-type-argument.title", "D",
    severity=Severity.ERROR,
)
def unexpected_type_argument(source: SourceFile) -> Iterable[Diagnostic]:
    ...
    if root is None:  # pragma: no cover - _parsed has already vetted the syntax
        return
    for mapping, key, parent in _owned_mappings(root):
        type_entry = _scalar_entries(mapping).get("Тип")
        if type_entry is None or not isinstance(type_entry[1], yaml.ScalarNode):
            continue
        value = type_entry[1].value.strip()
        if "<" not in value:
            continue
        ...
        yield Diagnostic(
            source.rel,
            type_entry[1].start_mark.line + 1, type_entry[1].start_mark.column + 1,
...
def _owned_mappings(root):
    """(mapping, the key it hangs on, the mapping that owns that key) for the whole document.

    A composed node does not know its parent, and the judgement needs one: a `Тип` value is
    read against the property of the ENCLOSING component. An item of a sequence inherits the
    key and the owner of the sequence itself - `Элементы:` holds its items directly.
    """
    ...
    while stack:
        ...
        if isinstance(node, yaml.MappingNode):
            if key is not None and parent is not None:
                yield node, key, parent
            for key_node, value_node in node.value:
                name = key_node.value if isinstance(key_node, yaml.ScalarNode) else None
                stack.append((value_node, name, node))
