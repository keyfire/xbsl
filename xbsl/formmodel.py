...
Model conventions (documented decisions):
    ...
    - The top-level `Свойства:` section (the component's own properties: a "-" list of
      ...
      the tree surfaces. The accepted spelling: the section follows the Наследует block, which is where property_add creates it.
"""
...
# Keys the model reads by name, in the spelling the model works in. A form may be written in
# English (`Content:`, `Type:`) - the platform reads it either way - so every such key is
# resolved through the compiler's own dictionary before it is compared. `Шапка` has no pair
# there and stays Russian-only: nothing proves an English spelling for it.
TYPE_KEY = "Тип"
NAME_KEY = "Имя"
ROOT_KEY = "Наследует"
PROPERTIES_KEY = "Свойства"
KIND_KEY = "ВидЭлемента"


@lru_cache(maxsize=None)
def _english_keys() -> dict[str, str]:
    """{English spelling: the Russian key} for the keys this module compares by name.

    Built FORWARD - the English name of each key we care about - because the dictionary is
    many-to-one in the other direction: `Type` is the English of both `Тип` and
    `ТипЭлементаПроекта`, and a plain reverse map would answer with whichever came last.
    """
    out: dict[str, str] = {}
...
def canonical_key(key: str) -> str:
    """The key as this module names it: an English spelling comes back Russian, the rest as is."""
    if not key or not key.isascii():
        return key
    return _english_keys().get(key, key)


def key_aliases() -> dict[str, str]:
    """{English spelling: the Russian key} of the form structure - for surfaces outside python.

    The form designer of the editor reads the yaml itself (a webview cannot call into the
    engine per node), so it needs the same pairs: without them an English form renders as an
    empty frame - the tree is looked up by `Содержимое` and the file spells `Content`.
    """
    return dict(_english_keys())
...
def _scalar_of(node: yaml.MappingNode, key: str) -> str | None:
    """The scalar value of a key, whichever of the two spellings the file uses."""
    for k, v in _mapping_pairs(node):
        if canonical_key(k.value) == key and isinstance(v, yaml.ScalarNode) and v.value:
            return str(v.value)
    return None
...
class _Builder:
    ...
    def build_component(
        ...
        for pair in node.pair_list:
            if canonical_key(pair.key) in CHILD_SLOTS and (
            ...
            elif canonical_key(pair.key) not in (TYPE_KEY, NAME_KEY):
            ...
            if isinstance(item, yaml.MappingNode):
                ...
                for k, v in _mapping_pairs(item):
                    if not isinstance(v, yaml.ScalarNode) or v.end_mark.index <= v.start_mark.index:
                        continue
                    if canonical_key(k.value) == NAME_KEY:
                        name_span = Span(v.start_mark.index, v.end_mark.index)
                    elif canonical_key(k.value) == TYPE_KEY:
...
def parse_form(text: str) -> Form:
    ...
    element_kind = _scalar_of(root_yaml, KIND_KEY)
    if element_kind:
        element_kind = metamodel.canonical_kind(element_kind)
    ...
    inherit_pair = next(
        ((k, v) for k, v in _mapping_pairs(root_yaml) if canonical_key(k.value) == ROOT_KEY), None
    )
    ...
    props_pair = next(
        ((k, v) for k, v in _mapping_pairs(root_yaml) if canonical_key(k.value) == PROPERTIES_KEY), None
    )
