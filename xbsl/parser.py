...
Design notes, verified empirically against the platform:
...
The parser must accept real-world sources with zero errors - like the token
...
MESSAGES = {
    ...
    "parser.expected-import-name": {
        ...
        "en": "Expected a subsystem name after '{n[импорт]}'",
    },
    ...
    "parser.expected-field-kw": {
        ...
        "en": "Expected '{n[знч]}', '{n[пер]}' or '{n[исп]}'",
    },
    ...
    "parser.expected-po-in-for": {
        ...
        "en": "Expected '{n[по]}' in the '{n[для]}' loop",
    },
    ...
    "parser.statement-not-expr": {
        "ru": "'{kw}' – это инструкция, а не выражение: значение даёт тернарный оператор "
              "'условие ? когда-истина : когда-ложь' либо присваивание внутри 'если'",
        "en": "'{kw}' is a statement, not an expression: a value comes from the ternary "
              "operator 'condition ? when-true : when-false' or from an assignment inside 'если'",
    },
    "parser.expected-type-after-new": {
        "ru": "Ожидается имя типа после 'новый'",
        "en": "Expected a type name after '{n[новый]}'",
    },
...
# Keywords that open a statement and are met in an EXPRESSION position often enough to
# deserve a message of their own (`знч Х = выбор ...` - the habit of another language).
_STATEMENT_KEYWORDS = frozenset({"IF", "CASE", "WHILE", "FOR", "TRY", "SCOPE", "RETURN"})
_COMPARE_OPS = ("<", "<=", "==", ">=", ">", "!=")
_ASSIGN_OPS = ("=", "+=", "-=", "*=", "/=")
...
class _Parser:
    ...
    def static_feature_resolving(self) -> Expr:
        ...
        if t.kind == "KEYWORD" and t.canonical in _STATEMENT_KEYWORDS:
            # `знч Х = выбор ...` is the classic BSL habit: in XBSL these keywords open a
            # STATEMENT, and "Ожидается выражение" says nothing about what to do instead.
            self.error(i18n.t("parser.statement-not-expr", kw=t.value))
