...
Conventions (documented decisions):
    ...
    - Inserting into a missing or empty slot follows the slot's CARDINALITY in the ui
      schema: a slot declared `Массив<...>` (Группа.Содержимое and friends) gets the "-"
      list form, everything else the single-mapping form, which is how the platform
      generators spell a singleton child. A single mapping in an array slot compiles into
      "Значение типа X не может быть присвоено в Массив<Компонент>" and takes the whole
      Компоненты.<Имя> resolution down with it. Where the schema cannot name the owner
      (a page item carries no Тип) or the data is absent, the single mapping stays.
...
def _plan_insert(form, parent, slot_name, item_fn, map_fn, before, after,
    ...
    if slot_node is None:
        ...
        body = (
            item_fn(col + step) if _slot_takes_a_list(parent, slot_name)
            else map_fn(col + step)
        )
        fragment, lead = eof_fix(pos, key_line + body)
    ...
    if not slot_node.children:  # "Слот:" with an empty value
        ...
        pos = slot_node.content_span.end
        body_col = slot_node.body_col + step
        body = (
            item_fn(body_col) if _slot_takes_a_list(parent, slot_name) else map_fn(body_col)
        )
        fragment, lead = eof_fix(pos, body)
        return _Plan([TextEdit(pos, pos, fragment)], pos + shift(pos) + lead)
...
def _split_mapping_chunks(lines: list[str]) -> list[list[str]]:
    ...
    with its own Тип line (the accepted spelling): the second and every following margin-0
...
def property_add(text: str, name: str, type_: str) -> EditResult:
    ...
    A new section lands right after the Наследует block - the accepted spelling
