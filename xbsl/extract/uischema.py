...
treats an absent key as false/null):
    ...
    { "meta": {"source": "docs", "element_version": "...", "tool": "extract_uischema",
      ...
      "type_params": {"ФрагментКомандногоИнтерфейса": {
          "params": ["ТипКоманды"],           # generic types, in declaration order
          "defaults": {"ТипКоманды": "Команда"}}} }   # only where the docs state one

The type_params section answers "is this the same type written differently":
`ФрагментКомандногоИнтерфейса<Команда>` IS `ФрагментКомандногоИнтерфейса`, because the
docs give ТипКоманды the default Команда, while `<ОбычнаяКоманда>` is another type and
the compiler rejects assigning it (a form's ДополнительныеКоманды, proven on a build).
A parameter with no documented default stays out - see param_defaults on why.
...
qualified names under "conflicts". The versions seen so far have no such name clashes.
...
#: The type parameters of a generic type, from its qualified name: `Пакет::Имя<ТипКоманды>`.
_TYPE_PARAMS_RE = re.compile(r"<(.+)>$")
#: The default of a type parameter, as the docs state it: "По умолчанию Команда",
#: "По умолчанию - Неопределено".
_PARAM_DEFAULT_RE = re.compile(
    r"По умолчанию\s*[-–—]?\s*([А-ЯЁA-Z][0-9A-Za-zА-Яа-яЁё_.]*)"
)
...
def declared_type_params(qualified: str) -> list[str]:
    """Type parameters of a generic type from its qualified name, in declaration order."""
    m = _TYPE_PARAMS_RE.search(qualified)
    return _split_top(m.group(1), ",") if m else []


def param_defaults(head_html: str, params: list[str]) -> dict[str, str]:
    """The documented default of each type parameter: `ТипКоманды: ... По умолчанию Команда.`

    Every parameter of a page is described in ONE paragraph, a sentence each, so the text is
    cut by the parameter names first - otherwise one parameter's default would be read as the
    next one's. A parameter whose default the docs do not state stays out of the answer:
    inventing one would make the consumers judge by a fact nobody stated.
    """
    out: dict[str, str] = {}
    for match in _P_RE.finditer(head_html):
        text = _plain(match.group(1))
        found = sorted((text.find(f"{name}:"), name) for name in params if f"{name}:" in text)
        if not found:
            continue
        bounds = [position for position, _ in found] + [len(text)]
        for index, (position, name) in enumerate(found):
            default = _PARAM_DEFAULT_RE.search(text[position:bounds[index + 1]])
            if default:
                out.setdefault(name, default.group(1).strip(" .,"))
    return out
...
class PageInfo:
    ...
    def __init__(self, rec: dict) -> None:
        ...
        #: Type parameters of a generic type, in declaration order, and the defaults the docs
        #: state for them (a parameter with no documented default is simply absent).
...
def parse_page(rec: dict) -> PageInfo:
    ...
    info.type_params = declared_type_params(info.qualified)
    if info.type_params:
        info.param_defaults = param_defaults(head, info.type_params)
...
def build_schema(pages: list[dict], element_version: str) -> dict:
    ...
    for page in sorted(infos, key=lambda p: p.id):
        if page.type_params and page.title not in type_params:
            rec = {"params": page.type_params}
            if page.param_defaults:
                rec["defaults"] = page.param_defaults
            type_params[page.title] = rec
    return {
        "meta": {
            "source": "docs",
        ...
        "enums": enums,
        "type_params": dict(sorted(type_params.items())),
    }


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        prog=_distro.prog_name("python -m xbsl.extract.uischema"),
