...
# Template directories that name NO element kind - the pages describe the types generated by a
# flavour of a kind or by a base of an interface component, and their members belong there.
...
def _template_kinds(car: zipfile.ZipFile) -> tuple[dict[str, str], list[str]]:
    ...
    """
    by_english = {en: ru for ru, en in scan_kind_table(car).items()}
    base = TEMPLATE_BASE
    ...
    names = sorted({
        ...
        if entry.startswith(base) and entry.endswith("/index.html")
    })
    kinds: dict[str, str] = {}
    unmapped: list[str] = []
    for name in names:
        if not name or name == "index":
            continue  # the index page of the template base itself
        if name in _TEMPLATE_KIND_EXCEPTIONS:
            kind = _TEMPLATE_KIND_EXCEPTIONS[name]
            if kind:
                kinds[name] = kind
            continue
        kind = by_english.get(name.removesuffix("Name"))
        if kind:
            kinds[name] = kind
        else:
...
def extract(dist: Path) -> tuple:
    ...
    with zipfile.ZipFile(car) as z:
        ...
        template_kinds, unmapped = _template_kinds(z)
        for name in unmapped:
            # Named, not swallowed: a template directory the rule cannot name a kind for is
        ...
        for n in (e for e in entries if e.startswith(TEMPLATE_BASE) and e.endswith("/index.html")):
            ...
            if kind is None:
                continue  # a template that names no kind
            if "." not in dirname:
