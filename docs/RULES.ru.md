...
названа другая система: `Менеджер сервиса`, `МС`, `1С:Предприятие`, `БСП` или `БТС` в пределах
двух слов. Сообщение подсказывает, как назвать систему, если ее имени рядом нет.
