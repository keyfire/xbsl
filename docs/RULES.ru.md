...
| `yaml/col-width-needs-no-stretch` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="info"><use href="#sev-info"/></svg> | – | файл | `Ширина` колонки таблицы – число или привязка – без `РастягиватьПоГоризонтали`: при растягивании число работает как доля свободного места, а не как пиксели [подробнее](#d-yaml-col-width-needs-no-stretch) [доки](https://1cmycloud.com/docs/help/topics/arrange-components-on-screen/) |
...
| `code/row-field-null` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="error"><use href="#sev-error"/></svg> | ✓ | проект | Поле строки, которое может быть `Null`, уходит туда, где объявленный тип его не принимает: поле динамического списка через ссылку (`Исполнитель.Номер`) в типизированное поле структуры или колонка `Запрос{...}`, прочитанная через ссылку или из присоединённой таблицы внешнего соединения, в поле структуры, параметр, переменную или результат метода (тип с `?` тоже не принимает `Null`). Компилятор отвечает `Null cannot be assigned` [доки](https://1cmycloud.com/docs/help/topics/dynamic-list/) |
...
| `yaml/double-quoted-binding` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="error"><use href="#sev-error"/></svg> | ✓ | файл | Привязка нестрокового свойства компонента в двойных кавычках: сервер такое значение отвергает, а голое значение или одинарные кавычки компилируются |
...
три вида колонок и ширина, заданная привязкой: во время работы она дает число. Колонка выходит
шире заданного, и содержимое уезжает от соседней. Пиксельной
...
Пометка это `РежимУдаления: ПометкаУдаления`, и она же умолчание. Свойство есть не у каждого
вида: у регистра его нет, и файл регистра правило не судит. Строку `Action УдалятьТекущий
cannot apply to object with a "DeletionMark"` говорит компилятор, в справке её нет.
...
членов.
При наличии свежих данных обобщенных типов контракт с ковариантным параметром покрывает более
широкие аргументы, и контракт для чтения, и изменяемый (`ИзменяемыйМассив<Объект>` покрывает
`Массив<Строка>`), а вложенные формулы баз подставляются. Конкретные `Массив`, `Соответствие`,
`Множество` и `Коллекция` сохраняют параметры инвариантными; без новых данных сохраняется
сравнение точных аргументов.
...
закомментированный код и цепочка другой системы не судятся. Не судится и имя, рядом с которым,
в пределах двух слов, названа другая система: `1С:Предприятие`, `БСП`, `БТС` или система, которую
проект объявил ключом `--other-system`. Сообщение подсказывает, как назвать систему, если ее
имени рядом нет.
