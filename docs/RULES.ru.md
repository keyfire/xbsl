...
список в рантайме – `xbsl --list-rules` (или MCP `list_rules`). Сейчас правил: 168.
...
| `yaml/unused-component` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="warning"><use href="#sev-warning"/></svg> | ✓ | проект | Компонент интерфейса, который нигде не размещён и не создан – ни значением `Тип` в разметке другого компонента, ни `новый` в коде. `code/unused-method` такой компонент не видит принципиально: его методы зовёт его же yaml. В yaml употреблением считается только ЗНАЧЕНИЕ (имя-ключ словаря локализации или перевода – не употребление), в модуле – любое слово текста, включая комментарий и строку. Не судятся точка входа (наследует клиентское приложение – её достаёт адрес) и компонент с `ОбластьВидимости: Глобально` – это публичная поверхность библиотеки. Прогон обязан покрывать проект: без файла-дескриптора (`Поставщик` + `Версия`) среди проверяемых файлов правило молчит – на подмножестве размещённый снаружи компонент выглядел бы мёртвым |
| `yaml/duplicate-subtree` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="warning"><use href="#sev-warning"/></svg> | – | проект | Поддерево разметки, дословно повторяющее устройство поддерева ДРУГОГО файла: имена, идентификаторы и тексты в слепок не входят – новую форму заводят копированием соседней, и копию переименовывают. Порог – 40 узлов: он выведен замером (на четырёх узлах живой проект отвечает 294 группами, на сорока – двумя настоящими копиями, чужой корпус – ни одной). Повтор внутри ОДНОГО файла не судится – зеркальные ветви формы это раскладка, а не копия; пропускаются источник данных списка (запрос и набор полей у списков одного вида совпадают по построению) и словарь локализованных строк (его языковой двойник повторяет устройство по определению). Называются только МАКСИМАЛЬНЫЕ группы. Выключено по умолчанию: мера одинаковости – решение проекта |
...
| `code/duplicate-method-body` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="warning"><use href="#sev-warning"/></svg> | – | проект | Тело метода дословно повторяется в ДРУГОМ файле: сравнивается нормализованное тело (без комментариев, пустых строк и отступов) от пяти строк. Платформенный хук отделяется по аннотации `@Обработчик`, а не по списку имён – одинаковое тело хука в каждом объекте нормально; копии внутри одного файла не судятся. Выключено по умолчанию: сводить ли две копии в один метод – решение проектировщика |
...
единственного. Нужен extra `[morph]` (`pip install "xbsl[morph]"`); без него русские имена молчат.
Английское имя (переведённое дерево) судится по последнему слову суффиксной эвристикой с перечнем
неправильных множественных, extra ему не нужен; несчисляемые слова и неоднозначные хвосты
...
Правда рантайма – `xbsl --list-rules`; таблица выше описывает только базовый набор.
