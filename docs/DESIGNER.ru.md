...
парой – выбор вкладки с одной стороны выводит вперёд другую, а закрытие панели закрывает yaml
формы и её модуль (если в них нет несохранённых правок). Сама пара вкладок ничего не добавляет
...
- **События.** У свойства-события – dropdown подходящих обработчиков модуля; "создать
  ...
  панелью – в отличие от клика по узлу, это явная просьба написать новый код, показать которую
  больше нечем, поэтому она открывает модуль даже из закрытого состояния. Сброс события
