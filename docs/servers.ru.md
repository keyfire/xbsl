...
У каждого инструмента `meta_*` есть параметр `root` – корень проекта вызывающего (агент в git
worktree не делит рабочий каталог с сервером): относительные `directory`, `yaml_path`,
`module_path` и подобные считаются от него, а ответ несёт абсолютные пути записанных файлов и
поле `root`, от которого их считали. Без параметра берётся рабочий каталог сервера, как раньше.
