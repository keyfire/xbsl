...
| Инструмент | Что делает |
|---|---|
| `translate_status(root, dictionary)` | покрытие и остаток – дешёвая проверка перед любым решением |
| `translate_gaps(root, kind, filter, limit, offset)` | чего словарь ещё не покрывает, страницами: частота, первые места, собственное написание платформы подсказкой |
| `translate_entries(root, kind, filter, limit, offset)` | что словарь уже говорит, с файлом и строкой каждой записи |
| `translate_set(root, entries, target, comment)` | запись: добавить, исправить на месте или снять, обнулив значение; `comment` – заголовок, с которым создаётся новый файл словаря |
