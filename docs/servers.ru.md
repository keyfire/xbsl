...
| `translate_gaps(root, kind, filter, limit, offset, compact)` | чего словарь ещё не покрывает, страницами: частота, первые места, собственное написание платформы подсказкой; `compact` оставляет в строке только ключ, вид и частоту |
