...
|---|---|
| `lint_paths(paths, select, ignore, enable, baseline, no_baseline, root, as_ci, as_ci_job)` | проверить файлы и каталоги на диске (относительные пути – от `root`, сводка его называет); базлайн проекта `.xbsllint-baseline` применяется сам, как и в CLI (`summary.baselined` – сколько погашено, `no_baseline` – показать и замороженные; устаревшие записи названы поимённо в `summary.baseline_stale_entries`, а не только сосчитаны); `enable` добавляет к умолчаниям правило, выключенное по умолчанию, – так проект спрашивает про пробелы словаря; `as_ci` берёт набор правил и базлайн из джобы CI проекта (`.gitlab-ci.yml` или воркфлоу GitHub рядом с проектом) – предполётная проверка тогда судит тем же, чем джоба, а сводка несёт `as_ci` (файл, джоба, ключи, прочие джобы с линтером); `as_ci_job` называет, КАКУЮ из них взять, – пайплайн, проверяющий второе дерево (перевод), гоняет линтер дважды и двумя разными наборами |
| `lint_source(filename, content, select, ignore)` | проверить содержимое в памяти, до записи файла |
| `baseline_prune(paths, select, ignore, enable, baseline, dry_run, root)` | снять записи базлайна, которые этому прогону больше не нужны (ключ CLI `--prune-baseline`): ответ называет каждую – путь, правило, сообщение, количество и причину, написанную человеком, – а файл сохраняет свой порядок и формат; записи правил, которых у сервера нет, и файлов вне `paths` не трогаются; `dry_run` показывает, что ушло бы |
