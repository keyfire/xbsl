...
| `translate_unused(root, kind, filter, limit, offset, prune, compact)` | обратный вопрос: что СЛОВАРЬ всё ещё говорит, а проекта уже нет. Удаление компонента навсегда оставляет за собой его имена и строки комментариев, и сказать об этом больше некому; `prune` снимает ровно ту страницу, которой отвечает, и по умолчанию выключен |
