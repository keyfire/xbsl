...
| `yaml/delete-current-needs-immediate` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="error"><use href="#sev-error"/></svg> | ✓ | file | `OnReferencedObjectDeletion: DeleteCurrent` on an attribute whose owner only marks a deletion: the apply rejects that pair [details](#d-yaml-delete-current-needs-immediate) [docs](https://1cmycloud.com/docs/help/topics/catalog-properties/) |
...
Marking is `DeletionMode: DeletionMark`, which is also the default. The apply answers `Action
DeleteCurrent cannot apply to object with a DeletionMark`.
