...
runtime is `xbsl --list-rules` (or the MCP `list_rules`). Currently there are 170 rules.
...
| `yaml/localization-missing-import` | <svg width="16" height="16" style="display:inline-block;vertical-align:-3px" aria-label="error"><use href="#sev-error"/></svg> | ✓ | project | An unqualified `$Dictionary.Key` whose dictionary lives in a subsystem this yaml does not import - the apply refuses the node as a not-imported namespace; an import in the paired module does not cover the markup, and the qualified `$Subsystem::Dictionary.Key` form needs no import [docs](https://1cmycloud.com/docs/help/topics/app-localization/) |
