...
**When the client says the transport closed.** The server keeps a short journal in the user's
...
against it, and the answer carries absolute paths for the written files and the diagnostics plus
the `root` they were resolved from. Without it the server's own working directory is used, as before. A relative
