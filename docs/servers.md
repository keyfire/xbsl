...
| `translate_unused(root, kind, filter, limit, offset, prune, compact)` | the opposite question: what the DICTIONARY still says and the project no longer has. Deleting a component leaves its names and comment lines behind for good, and nothing else reports them; `prune` removes exactly the page it answers with, and is off by default; `compact` keeps only the key, the kind, the file and the line of each row, and every answer counts the orphans by kind in `counts` |
