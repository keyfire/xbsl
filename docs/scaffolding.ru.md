...
получает вид `$Словарь.Имя`, и нужные ключи в той же операции добавляются в словарь и в его
...
`set-localization` записывает ОДНУ строку локализации во все языки сразу: текст языка по
умолчанию – в сам элемент `ЛокализованныеСтроки` (там его и держит платформа), каждый
другой – в свой `Локализация/<Код>/<Имя>.yaml`. Это недостающая половина
`add-localization`, который добавляет ЯЗЫК: заголовок набирали в элементе и ещё раз в его
английском двойнике, и файлы расходились молча – сверить их было нечем, кроме глаз. Язык,
названный без файла перевода, отвергается (сначала добавьте язык); язык, о котором вызов
молчит, всё равно получает строку – значением языка по умолчанию и с заметкой, – чтобы ни
один перевод не остался на ключ короче. Ключ сохраняет секцию, в которой уже живёт, а
новый ложится в `Строки`, если `--section` не говорит `Шаблоны`.

`add-field --kind строка` (`meta_add_field`) добавляет КЛЮЧ: он ложится в элемент и
дописывается в готовые переводы значением языка по умолчанию, чтобы ни один из них не остался
на ключ короче; заметка называет `set-localization` – тем текст перевода и пишется. Вызов по
САМОМУ файлу перевода отвергается: у него нет ни вида, ни секций элемента, и отказ называет
элемент, к которому файл относится, и тот же `set-localization` вместо прежнего "у вида ? нет
секции".
