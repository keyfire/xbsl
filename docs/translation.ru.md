...
кириллическая строка комментария переводятся людьми. Комментарий файла ресурса идёт тем же планом
фраз, что и комментарий модуля. Три плана:
...
## Записанное дерево – репозиторий

`--out КАТАЛОГ` считает КАТАЛОГ корнем РЕПОЗИТОРИЯ: проект ложится в `КАТАЛОГ/{Поставщик}/{Имя}`
– в те два имени, которые несёт переведённый дескриптор. Другой раскладки сборка не принимает:
она пакует файлы по схеме `{поставщик}/{имя}` и каталог с другим именем отвергает, – поэтому
дерево выкатывается как есть, без ручной перекладки. Имена берутся ПЕРЕВЕДЁННЫЕ: проект, чьё имя
само по себе русское слово, меняет его в проходе, и каталог следует за ним.
...
попросили. Строка лога называет каталог, в который файлы легли на самом деле.

Повторный прогон в тот же каталог просто переписывает дерево: каталог узнаётся по дескриптору
(`Project.yaml`/`Проект.yaml`). Каталог, занятый ЧУЖИМИ файлами, не трогается вовсе – прогон
говорит, какой это каталог и что с ним делать. Отдельный файл, который записать не удалось
(остаток прошлого прогона: каталог на месте файла, файл только для чтения, файл занят другой
программой), называется в отчёте вместе с причиной, а не роняет проход: отчёт печатается
целиком, первые пять таких файлов названы поимённо, остальные сосчитаны. Прогон, которому не
удалось записать дерево, возвращает ненулевой код и без `--strict`: дерево – это и есть его
работа.

**`--clean`: остатки прошлого прогона.** Перезапись покрывает файлы, которые прогон пишет, и
больше ничего не трогает, поэтому файл, ПЕРЕИМЕНОВАННЫЙ или снятый в исходнике, оставляет своё
старое отражение в дереве вывода – сборка берёт каталог целиком, и сирота уезжает вместе со
всем остальным. Хуже того, остаток может стоять ровно на месте нужного файла: каталог вместо
файла – это и есть ошибка записи выше. `--clean` убирает перед записью всё, чего этот прогон не
пишет:
...
Ключ явный, потому что удаляет файлы, и это НЕ запись во временный каталог с подменой: подмена
всё равно обязана удалить старое дерево, стоит вторую полную копию проекта и ломается ровно на
тех условиях, из-за которых и случаются ошибки записи, – другой том, каталог, занятый сборкой.
Отказ каталогу с чужими файлами остаётся страховкой сверху: чистка идёт только внутри каталога,
который УЖЕ является переведённым проектом, так что `--clean` не превращается в "сотри то, на
что показали". Неудавшееся удаление называется проблемой так же, как неудавшаяся запись; число
убранного печатается в отчёте (и в поле `removed` у json) только тогда, когда убирать было что,
а под ним перечисляется каждый остаток поимённо – число ничего не отвечает о том, что именно
потеряла сборка.

**`--dry-run`: посмотреть до того, как случится.** Первая чистка переведённого дерева иначе
делается вслепую. Ключ доводит прогон до записи и останавливает: дерево построено, каталог
назначения проверен (занятый чужими файлами отвергается здесь так же, как отверг бы настоящий
прогон), остатки перечислены, – а записывать и удалять нечего.
...
(размер дерева, которое прогон записал бы) и `removals` – все до одного. Без `--out` описывать
...
увидеть это может только транслятор. Каждое столкнувшееся имя названо со своим местом –
`method:СтрокаРолей - 'Number' <- ... (Модуль.xbsl:2:9), ... (Модуль.xbsl:5:13)`, – потому что
одно пространство имён оставляет читателю поиск двух слов среди полутора десятков, объявленных
методом, а рядом они стоят редко.

Ещё две проблемы приходят из самого словаря, и обе найдены на живом проекте, английская сборка
которого не собиралась при покрытии 100%. Запись, которая пишет член платформы так, как платформа
не пишет его нигде (`Важность: Severity` против `Importance` у события), сообщается в первом
месте, где это доказывает приёмник ИЗВЕСТНОГО типа: там берётся платформенное написание, а
приёмник, тип которого никто не назвал, получит слово из записи, и компилятор его отвергнет, –
лечение одно: платформенное написание в записи. Слово, которое сама платформа пишет двумя
способами (`Загрузить` – `Load` у двоичного объекта и `Upload` у хранилища объектов), не судится:
запись, совпадающая с любым из них, ничего не ломает. И именованный литерал, подстановки перевода
которого после перевода не совпадают с подстановками ключа (`%{AccountCode}` там, где поле
переводится как `SubscriberCode`), сообщается с обоими списками: имена внутри `%{...}` должны
переводить те же поля, на любом из двух языков.
...
`--since` отвечает на вопрос, который задача задаёт в конце: не что накопилось в словаре за всю
жизнь проекта, а что осталось от ЕЁ правки. В ответе – ключи, которые проект больше не пишет
нигде И встречались только в строках, снятых этой правкой; `--prune` вместе с ним снимает ровно
их. Ветка или коммит читаются от точки расхождения с HEAD до РАБОЧЕГО ДЕРЕВА, поэтому
незакоммиченное тоже считается правкой; диапазон `A..B` передаётся git как написан – так
смотрят на уже слитую правку. Замер на живом проекте: без отбора 3297 сирот, из них правке
принадлежат 18 – девять имён и девять строк комментариев, одним вызовом вместо девятнадцати
вызовов с `--filter` по одному имени.

Сужение – пересечение, а не короткий путь: имя, которое проект всё ещё пишет, сиротой не
станет, как бы щедро ни прочитался diff. Поэтому ответ без отбора и без `--since` несёт
оговорку: чтение текстовое, список описывает весь накопленный словарь и годится на просмотр,
а не на снятие целиком.
...
называется. Строка комментария читается разбором самого переводчика – маркер и обрамление
снимаются ровно так же, как их снимает записывающий проход, – а имя ищется и в ИМЕНАХ ФАЙЛОВ:
папка и файл проходят через ту же плоскость имён. Квалифицированный ключ (`<Владелец>.<Имя>`) судится по обоим концам: в исходниках
они написаны порознь, и чтение точечного текста как одного имени объявило бы сиротой каждую
такую пару.

`--redundant` – второе зеркало `--gaps`: не слово, которого словарю не хватает, а слово,
которое он пишет ровно так, как его пишет сама ПЛАТФОРМА. Такая пара ничего не ломает –
дерево выходит без неё слово в слово тем же, – и потому её и стоит назвать: она отвечает
вместо данных платформы, а значит, прячет за собой пробел этих данных или самого движка.
В одном живом словаре так были написаны языки описания проекта, и наполовину переведённое
перечисление за этой парой нашёл тест на пустом словаре, а не проект. Обычный отчёт называет
их число, ни о чём не спрашивая; `--prune` снимает ровно те строки, которые флаг перечислил.

В отличие от `--unused` здесь идёт ПРОХОД по проекту, и вердикт – свидетельство, а не второе
чтение таблиц: пара попадает в список, только если КАЖДОЕ место, где она ответила, вышло бы
тем же самым и без неё. Поэтому пара, которая держит хотя бы одно своё место – слово, которое
платформа пишет в одной роли и не пишет в другой, – лишней не называется, а пара, которой
проект не пользуется вовсе, сюда не попадает: это вопрос о сиротах, на него отвечает
`--unused`.
...
выглядит полным ответом, и собранный по такой странице словарь оказался короче на
двадцать две записи – их нашёл строгий проход уже после слияния. Поэтому урезанная
страница несёт `truncated: true`, `remaining` и `hint` со следующим `offset`, `limit=0`
для всего списка и – у пропусков – `--missing`, который записывает весь остаток файлом
заготовки словаря.
