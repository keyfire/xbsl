...
- `translate_unused` – обратный вопрос: что словарь всё ещё говорит, а проекта уже нет;
  `filter` сужает ответ до имён одного удалённого компонента, `prune` (по умолчанию
  выключен) снимает ровно ту страницу, которой инструмент отвечает, `compact` оставляет
  только ключ, вид, файл и строку, а `counts` считает сирот по видам;
