...
either combines with a single `name` argument. A file that several keys touch is read once and
...
than leaving the ones before it already on disk. MCP `meta_set_localization` takes the same
