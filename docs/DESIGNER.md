...
- **1C:Element • Project** – the primary side bar on the left: the metadata tree and
  the **Palette** (the latter only while the form panel is open).
- **1C:Element • Inspector** – the secondary side bar on the right: the
  **Properties** and **Documentation** panels.
